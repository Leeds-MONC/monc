...
module dephy_forcings_mod
  ...
  ! Forcing fields on MONC vertical grid
  real(kind=DEFAULT_PRECISION), allocatable :: ug_dephy(:,:), &
    vg_dephy(:,:), &
    u_adv_dephy(:,:), &
    v_adv_dephy(:,:), &
    theta_adv_dephy(:,:), &
...
contains
  ...
  type(component_descriptor_type) function dephy_forcings_get_descriptor()
    ...
    dephy_forcings_get_descriptor%finalisation=>finalise_callback
  ...
  subroutine initialise_callback(current_state)
    ...
    !call dephy_read_flags() ! reads flags
    !call dephy_initialise_profiles(current_state)
  ...
  subroutine timestep_callback(current_state)
    ...
    !call dephy_update_surface_variables(current_state)
  ...
  subroutine dephy_read_dimension_variable(field,netcdf_name)
    implicit none
    real(kind=DEFAULT_PRECISION), allocatable, intent(inout) :: field
    logical :: dim_found
    integer :: dim_length

    call check_status(nf90_inq_dimid(ncid_dephy, netcdf_name, this_dim_length), dim_found)
    if not(dim_found)
       stop 'did not find dimension '//netcdf_name
    else
       call check_status(nf90_inquire_dimension(ncid_dephy, netcdf_name, len=dim_length))
    end if
  ...
  subroutine dephy_read_profile_variable(field,netcdf_name,ncid)
    implicit none


  end subroutine dephy_read_profile_variable

  subroutine dephy_read_forcing_variable(field,netcdf_name,ncid)
    implicit none

  end subroutine dephy_read_forcing_variable

  subroutine dephy_read_flag(flag,netcdf_name,ncid)
    implicit none
    logical, intent(inout) :: flag

  end subroutine dephy_read_flag

  subroutine dephy_read_dimension_variables
    implicit none
    call dephy_read_dimension_variable(time_dephy,'time')
    call dephy_read_height_variable(height_dephy,'lev')
  ...
  subroutine dephy_read_profile_variables
    ...
    call dephy_read_profile_variable(height_forc_dephy,'height_forc')
    call dephy_read_profile_variable(pressure_dephy, 'pressure')
  ...
  subroutine dephy_read_forcing_variables
    ...
    call dephy_read_forcing_variable(w_dephy, 'w_dephy')
  ...
  subroutine dephy_read_surface_variables
    ...
    call dephy_read_surface_variable(ps_dephy, 'ps')
    call dephy_read_surface_variable(sfc_sens_flx_dephy, 'sfc_sens')
    call dephy_read_surface_variable(sfc_lat_flx_dephy, 'sfc_lat')
    call dephy_read_surface_variable(z0_traj_dephy, 'z0')
    call dephy_read_surface_variable(z0th_traj_dephy, 'z0th')
    ...
    call dephy_read_surface_variable(albedo_traj_dephy, 'albedo')
    call dephy_read_surface_variable(q_skin_traj_dephy, 'q_skin')

  end subroutine dephy_read_surface_variables

  subroutine dephy_read_flags
    implicit none

    call dephy_read_flag(l_adv_theta, 'adv_theta')
    call dephy_read_flag(l_adv_rv, 'adv_rv')
    call dephy_read_flag(l_rad_theta, 'rad_theta')
    call dephy_read_flag(l_forc_omega, 'forc_omega')
    call dephy_read_flag(l_forc_w, 'forc_w')
    call dephy_read_flag(l_forc_geo, 'forc_geo')
    call dephy_read_flag(l_nudging_u, 'nudging_u')
    call dephy_read_flag(l_nudging_v, 'nudging_v')
    call dephy_read_flag(l_nudging_theta, 'nudging_theta')
    call dephy_read_flag(l_nudging_rv, 'nudging_rv')

  end subroutine dephy_read_flags

  subroutine dephy_apply_forcings
    implicit none

  end subroutine dephy_apply_forcings

  subroutine dephy_update_surface_variables
    implicit none

  end subroutine dephy_update_surface_variables

  subroutine dephy_initialise_profiles
    implicit none

  end subroutine dephy_initialise_profiles
