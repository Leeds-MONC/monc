...
module iterativesolver_single_prec_mod
  ...
  use datadefn_mod, only : DEFAULT_PRECISION, PRECISION_TYPE, &
      SINGLE_PRECISION, SINGLE_PRECISION_TYPE
  use logging_mod, only : LOG_WARN, LOG_DEBUG, log_log, log_get_logging_level
  use conversions_mod, only : conv_to_string
  use communication_types_mod, only : halo_communication_single_prec_type, halo_communication_type, &
    neighbour_description_single_prec_type, & 
    field_data_wrapper_single_prec_type
  use halo_communication_single_mod, only : copy_buffer_to_field, copy_field_to_buffer, perform_local_data_copy_for_field, &
  ...
  type matrix_type
     real(kind=SINGLE_PRECISION) :: n, s, e, w
     real(kind=SINGLE_PRECISION), dimension(:), allocatable :: u, d, p, lu_d, lu_u, vol
  end type matrix_type

  real(kind=SINGLE_PRECISION) :: tolerance, relaxation     !< Solving tollerance
  ...
  real(kind=SINGLE_PRECISION), parameter :: TINY = 1.0e-16 !< Minimum residual - if we go below this then something has gone wrong

  type(halo_communication_single_prec_type), save :: halo_swap_state      !< The halo swap state as initialised by that module
  real(kind=SINGLE_PRECISION), dimension(:,:,:), allocatable :: psource, prev_p, p_data     !< Passed to BiCGStab as the RHS
  logical :: first_run=.true.
  type(matrix_type) :: A

  public iterativesolver_single_prec_get_descriptor
contains

  !> Descriptor of the iterative solver component used by the registry
  !! @returns The iterative solver component descriptor
  type(component_descriptor_type) function iterativesolver_single_prec_get_descriptor()
    iterativesolver_single_prec_get_descriptor%name="iterativesolver_single_prec"
    iterativesolver_single_prec_get_descriptor%version=0.1
    iterativesolver_single_prec_get_descriptor%initialisation=>initialisation_callback
    iterativesolver_single_prec_get_descriptor%timestep=>timestep_callback
    iterativesolver_single_prec_get_descriptor%finalisation=>finalisation_callback
  end function iterativesolver_single_prec_get_descriptor
  ...
  subroutine initialisation_callback(current_state)
    ...
    allocate(psource(current_state%local_grid%size(Z_INDEX) + current_state%local_grid%halo_size(Z_INDEX) * 2, &
         ...
         current_state%local_grid%size(Y_INDEX) + current_state%local_grid%halo_size(Y_INDEX) * 2, &
         current_state%local_grid%size(X_INDEX) + current_state%local_grid%halo_size(X_INDEX) * 2), &
         p_data(current_state%local_grid%size(Z_INDEX) + current_state%local_grid%halo_size(Z_INDEX) * 2, & 	 	
	       current_state%local_grid%size(Y_INDEX) + current_state%local_grid%halo_size(Y_INDEX) * 2, & 	 	
         current_state%local_grid%size(X_INDEX) + current_state%local_grid%halo_size(X_INDEX) * 2))
  ...
  subroutine timestep_callback(current_state)
    ...
    psource=real(current_state%p%data, kind=SINGLE_PRECISION)
    if (first_run) then
      ! If first timestep then initial guess is zero
      current_state%p%data=0.0_DEFAULT_PRECISION
      p_data=real(current_state%p%data, kind=SINGLE_PRECISION)
      first_run=.false.
    else
    ...
    if (symm_prob) then
      call cg_solver(current_state, A, p_data, psource, i_strt, i_end, j_strt, j_end, k_end)
    else
      call bicgstab(current_state, A, p_data,  psource, i_strt, i_end, j_strt, j_end, k_end)
    end if

    prev_p=p_data
    current_state%p%data = real(p_data, kind=DEFAULT_PRECISION)
  ...
  subroutine bicgstab(current_state, A, x, b, i_strt, i_end, j_strt, j_end, k_end)
    ...
    real(kind=SINGLE_PRECISION), dimension(:,:,:), intent(inout) :: x
    real(kind=SINGLE_PRECISION), dimension(:,:,:), intent(in) :: b
    integer, intent(in) :: i_strt, i_end, j_strt, j_end, k_end

    integer :: it, i, j, k
    real(kind=SINGLE_PRECISION) :: sc_err, alf, omg, nrm, my_rho, bet, tt, ts, ss, err, init_err, inner_prod_results(3)
    real(kind=SINGLE_PRECISION), dimension(current_state%local_grid%size(Z_INDEX) + &
    ...
    sc_err = max(sc_err, 0.0001_SINGLE_PRECISION)
    ...
    alf = 1.0_SINGLE_PRECISION
    omg = 1.0_SINGLE_PRECISION
  ...
  subroutine cg_solver(current_state, A, x, b, i_strt, i_end, j_strt, j_end, k_end)
    ...
    integer :: it, k, i, j
    real(kind=SINGLE_PRECISION) :: sc_err, alf, bet, err, init_err, rho
    real(kind=SINGLE_PRECISION), dimension(current_state%local_grid%size(Z_INDEX) + &
         current_state%local_grid%halo_size(Z_INDEX) * 2, &
         current_state%local_grid%size(Y_INDEX) + current_state%local_grid%halo_size(Y_INDEX) * 2, &
  ...
  subroutine precond(current_state, A, s, r, preits)
    type(model_state_type), target, intent(inout) :: current_state
    real(kind=SINGLE_PRECISION), dimension(:,:,:), intent(in) :: r
    real(kind=SINGLE_PRECISION), dimension(:,:,:), intent(inout) :: s
    ...
    real(kind=SINGLE_PRECISION), dimension(current_state%local_grid%size(Z_INDEX)) :: s_k
    integer :: it, i, j, k
  ...
  subroutine calc_Ax(current_state, A, x, Ax)
    ...
    real(kind=SINGLE_PRECISION), dimension(:,:,:), target, intent(inout) :: x, Ax

    integer :: i, k, j, n, istart, iend, jstart, jend
    type(field_data_wrapper_single_prec_type) :: source_data   
  ...
  real(kind=SINGLE_PRECISION) function inner_prod(current_state, x, y, i_strt, i_end, j_strt, j_end, k_end)
    type(model_state_type), target, intent(inout) :: current_state
    real(kind=SINGLE_PRECISION), dimension(:,:,:), intent(in) :: x, y
    integer, intent(in) :: i_strt, i_end, j_strt, j_end, k_end

    real(kind=SINGLE_PRECISION) :: global_sum
    real(kind=DEFAULT_PRECISION) :: local_sum, local_sum_mpi, global_sum_mpi
    integer :: ierr, i, j, k

    local_sum=0.0_DEFAULT_PRECISION
    local_sum_mpi = 0.0_DEFAULT_PRECISION

     do i=i_strt, i_end
      do j=j_strt, j_end
        do k=2, k_end
          local_sum=local_sum+real(x(k,j,i), kind=DEFAULT_PRECISION)* &
               real(y(k,j,i), kind=DEFAULT_PRECISION)
        end do        
      end do      
    end do

    
    call mpi_allreduce(local_sum, global_sum_mpi, 1, PRECISION_TYPE, MPI_SUM, &
      current_state%parallel%monc_communicator, ierr)
    
      global_sum = real(global_sum_mpi, kind=SINGLE_PRECISION)
  ...
  function inner_prod_three_way(current_state, t, s, i_strt, i_end, j_strt, j_end, k_end)
    type(model_state_type), target, intent(inout) :: current_state
    integer, intent(in) :: i_strt, i_end, j_strt, j_end, k_end
    real(kind=SINGLE_PRECISION), dimension(:,:,:), intent(in) :: t, s
    real(kind=SINGLE_PRECISION), dimension(3) :: inner_prod_three_way

    real(kind=SINGLE_PRECISION), dimension(3) :: global_sum
    real(kind=DEFAULT_PRECISION), dimension(3) :: local_sum, local_sum_mpi, global_sum_mpi 
    integer :: ierr, i, j, k
    ...
    local_sum_mpi(1)=0.0_DEFAULT_PRECISION
    local_sum_mpi(2)=0.0_DEFAULT_PRECISION
    local_sum_mpi(3)=0.0_DEFAULT_PRECISION

    do i=i_strt, i_end
      do j=j_strt, j_end
        do k=2, k_end
          local_sum(1)=local_sum(1)+real(t(k,j,i), kind=DEFAULT_PRECISION) &
               *real(t(k,j,i), kind=DEFAULT_PRECISION)
          local_sum(2)=local_sum(2)+real(t(k,j,i), kind=DEFAULT_PRECISION) & 
               *real(s(k,j,i), kind=DEFAULT_PRECISION)
          local_sum(3)=local_sum(3)+real(s(k,j,i), kind=DEFAULT_PRECISION) &
               *real(s(k,j,i), kind=DEFAULT_PRECISION)
        end do
      end do
    end do

    call mpi_allreduce(local_sum, global_sum_mpi, 3, PRECISION_TYPE, MPI_SUM, &
      current_state%parallel%monc_communicator, ierr)
  ...
  subroutine set_matrix_for_poisson(grid_configuration, A, z_size)
    ...
    integer :: k
    real(kind=SINGLE_PRECISION) :: d_sc, concat_scalars    
    ...
    do k=2, z_size      
     ...
     k=2 
     A%lu_d(k)=1.0_SINGLE_PRECISION/A%p(k) 
     A%lu_u(k)=A%lu_d(k)*A%u(k) 
     do k=3, z_size 
       A%lu_d(k)=1.0_SINGLE_PRECISION/(A%p(k) - A%d(k)*A%lu_u(k-1))
  ...
  subroutine copy_p_to_halo_buffer(current_state, neighbour_description, dim, source_index, &
    ...
    type(neighbour_description_single_prec_type), intent(inout) :: neighbour_description
    type(field_data_wrapper_single_prec_type), dimension(:), intent(in), optional :: source_data

    call copy_field_to_buffer(current_state%local_grid, neighbour_description%send_halo_buffer, p_data, &
  ...
  subroutine copy_calc_Ax_to_halo_buffer(current_state, neighbour_description, dim, source_index, &
    ...
    type(field_data_wrapper_single_prec_type) :: selected_source
  ...
  subroutine copy_halo_buffer_to_p(current_state, neighbour_description, dim, target_index, &
    ...
    call copy_buffer_to_field(current_state%local_grid, neighbour_description%recv_halo_buffer, p_data, &
  ...
  subroutine copy_halo_buffer_to_calc_Ax(current_state, neighbour_description, dim, target_index, &
    ...
    call copy_buffer_to_field(current_state%local_grid, &
        neighbour_description%recv_halo_buffer, selected_source%data, &
        dim, target_index, current_page(neighbour_location))
  ...
  subroutine perform_local_data_copy_for_p(current_state, halo_depth, involve_corners, source_data)
    ...
    call perform_local_data_copy_for_field(p_data, current_state%local_grid, &
...
end module iterativesolver_single_prec_mod
